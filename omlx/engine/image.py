...
from ..image_registry import (
    ...
    image_model_requires_local_path,
...
def _image_api() -> _ImageAPI:
    """Import mlx-vlm's generic image API only when an image engine is loaded."""
    try:
...
class ImageEngine(BaseNonStreamingEngine):
    ...
    async def start(self) -> None:
        """Validate metadata and load the primary task's model variant."""
        if self._started:
            return
        ...
        try:
            async with self._call_lock:
                ...
                request = api.ImageGenerationRequest(
                    ...
                    width=512 if width is None else int(width),
                    height=512 if height is None else int(height),
                    guidance=self._resolve_guidance(guidance, "generation"),
                    output_format="png",
                    extra=self._request_extra(kwargs),
                )
                raw_result = await self._run_api_call(
                    api.generate_image, model, request, task="generate"
                )
        ...
        try:
            async with self._call_lock:
                ...
                image_strength = self._resolve_image_strength(
                    kwargs.pop("image_strength", None), "edit"
                )
                ...
                raw_result = await self._run_api_call(
                    api.generate_image, model, request, task="edit"
                )
                return self._result_from_raw(
                    raw_result,
                    task="edit",
    ...
    def _resolve_model_reference(self, override: object = None) -> str:
        ...
        if value is None:
            if not root.exists() and not self._has_explicit_model_path:
                spec = get_image_model_spec(self.base_model)
                if image_model_requires_local_path(self.base_model):
                    raise ValueError(
                        f"{self.base_model} requires a converted local MLX model path"
                    )
                return (
    ...
    def _model_key(
        self, task: ImageTask, model_path_override: object = None
    ) -> _ModelKey:
        return _ModelKey(
            task=task,
        ...
        async with self._load_lock:
            ...
            try:
                model = await loop.run_in_executor(
                    get_mlx_executor(),
                    lambda: api.load_image_model(
                        key.model_reference, task=loader_task
                    ),
                )
    ...
    async def _run_api_call(
        generate_image: Any, model: Any, request: Any, *, task: str
    ) -> Any:
        loop = asyncio.get_running_loop()
