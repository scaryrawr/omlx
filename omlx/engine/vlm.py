...
class VLMBatchedEngine(BaseEngine):
    ...
    def _image_token_count(self, input_ids: Any) -> int | None:
        config = getattr(self._vlm_model, "config", None)
    ...
    def _process_chat_messages(
        ...
        if model_type in {"mimo_v2", "mimo_v2_flash"}:
            media_messages = expand_video_parts(messages)
        text_messages, images, audio, videos = extract_media_from_messages(media_messages)
        try:
    ...
    def count_chat_tokens(
        ...
        """
        # MiMo videos must be sampled before the generic extractor sees them.
        media_messages = messages
        model_type = self.model_type or _read_config_model_type(self._model_name)
        if model_type in {"mimo_v2", "mimo_v2_flash"}:
            media_messages = expand_video_parts(messages)
        text_messages, _, _, videos = extract_media_from_messages(media_messages)
        try:
            template_tools = convert_tools_for_template(tools) if tools else None
            prompt = self._apply_chat_template(
                text_messages,
