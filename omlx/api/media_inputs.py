...
def normalize_media_file_part(part: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if url:
        raise ValueError(
            "file_url is not supported for audio/video file parts. "
            "Send base64 content in file_data."
        )
    if file_obj.get("file_id") and not data:
        raise ValueError(
            "File IDs for audio/video inputs are not supported. "
            "Send base64 content in file_data."
        )
    if not data:
        raise ValueError("Audio/video file parts require base64 file_data.")
...
def normalize_media_file_parts_in_messages(messages: list[Any]) -> list[Any]:
    ...
    for msg in messages:
        ...
        for part in content:
            ...
            media_part = normalize_media_file_part(part_dict)
