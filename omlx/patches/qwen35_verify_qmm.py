...
Supported: 4-bit and 8-bit affine, MXFP4, and MXFP8 with bf16/fp16 activations
and M in 3..6. The mma path takes 4-bit and 5-bit affine with M in 7..24.
Everything else falls back to stock.
"""
...
def apply_verify_qmm_patch() -> bool:
    ...
    fall in 3..6 (split-K) or 7..24 (mma), only layouts the kernels support.
    ...
    def patched_call(self, x):
        ...
        route = None
        if mode == "affine" and mma_eligible(
            rows, K, N, self.bits, self.group_size, x.dtype
        ):
            route = "mma"
        elif rows <= 6 and vk_eligible(
            rows, K, N, self.bits, self.group_size, x.dtype, mode
        ):
            route = "vk"
        if route is None:
            return orig_call(self, x)
    ...
    _QL_PATCHED = True
    logger.info("MTP verify qmm patch applied (rows 3..6 affine/MXFP split-K, 7..24 affine mma)")
    return True
