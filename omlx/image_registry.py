...
IMAGE_MODEL_SPECS: tuple[ImageModelSpec, ...] = (
    ...
    ImageModelSpec(
        ...
        discovery_aliases=_aliases("z-image", "zimage", "tongyi-mai/Z-Image"),
...
def image_model_requires_local_path(base_model: str) -> bool:
    """Return whether mlx-vlm requires converted local weights for this family."""
    spec = get_image_model_spec(base_model)
    return bool(spec and spec.requires_local_path)
...
IMAGE_TASK_DEFAULTS: dict[tuple[str, ImageTask], dict[str, int | float]] = {
    ...
    ("ernie-image-turbo", "edit"): {"default_guidance": 3.0},
}
