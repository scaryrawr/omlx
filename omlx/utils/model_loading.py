...
def _is_mtp_weight_key(key: str) -> bool:
    return (
        key.startswith(_MTP_WEIGHT_PREFIXES) or _MTP_WEIGHT_RE.search(key) is not None
    )
...
def _checkpoint_has_mtp_weights(model_path: str | Path) -> bool:
    ...
    for index_path in _iter_safetensors_index_paths(p):
        try:
            ...
            if any(_is_mtp_weight_key(str(k)) for k in weight_map):
                return True
        except Exception as e:
...
def _jang_mxtq_bits_with_codec(
    sources: tuple[Any, ...], codec: JangCodec
) -> Any | None:
...
def _classify_jang_codec(
    ...
    explicit_jangtq = any(
        marker in {"affinemxtq", "jangtq", "mxtq"} or marker.startswith("jangtq")
...
def patch_jang_quantization_from_shapes(
    ...
    for key, (bits, group_size) in sorted(inferred.items()):
        should_override = (bits, group_size) != (default_bits, default_group_size)
        existing = quantization.get(key)
        if isinstance(existing, dict):
            existing_pair = _quantization_default_pair(existing)
            should_override = should_override or existing_pair != (bits, group_size)
...
def read_jang_metadata(model_path: str | Path) -> JangQuantizationMetadata | None:
    ...
    return JangQuantizationMetadata(
        ...
        mxtq_bits=_jang_mxtq_bits_with_codec(sources, codec),
    ...
    try:
        ...
        if bundle_path is not None:
            try:
                ...
                return cast(tuple[Any, Any], load_jang_model_from_bundle(bundle_path))
        ...
        with _scoped_jang_source_tokenizer_fallback(jang_metadata, load_path):
            if jang_metadata.codec == "jangtq" and is_vlm:
                if jang_metadata.is_kimi_vlm:
                    ...
                    return _retain_jang_loader_temp_dir(
                        cast(tuple[Any, Any], load_jangtq_kimi_vlm_model(load_path)),
                        temp_dir,
                    )
