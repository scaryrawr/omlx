...
_MODEL_SHARD_RE = re.compile(r"^model-(\d+)-of-(\d+)\.safetensors$")
...
def select_safetensors_weight_files(model_path: Path) -> list[Path]:
    ...
    ``model-NNNNN-of-MMMMM.safetensors`` sets in the same directory. When a
    complete numbered set exists, this helper returns only the largest complete
    set so header scans and size estimates do not mix stale and current shards.
    Non-numbered single-file checkpoints are returned unchanged.
    """
